...
def try_replace(string, needle, multi=False):
	tr = string
	...
	if sfn != -1 and not in_tag(string, sfn) and not in_tag(string, sfn + len(needle)) and (len(string) == sfn + len(needle) or string[sfn + len(needle)] in " .,:;\"\'"): 
		tr = string.replace(needle, replacement_list[needle], 1)
	if multi:
		# if multi keep trying till you run out of string
		while sfn + len(replacement_list[needle]) < len(tr) and needle in tr[sfn + len(replacement_list[needle]):]:
			...
			if sfn != -1 and not in_tag(tr, sfn) and not in_tag(tr, sfn + len(needle)) and (len(tr) == sfn + len(needle) or tr[sfn + len(needle)] in " .,:;\"\'"): 
				tr = tr[:sfn] + tr[sfn:].replace(needle, replacement_list[needle], 1)

	return tr

def run_replaceemnt_list(string, ignore_list=[], multi=False):
	for needle in replacement_list:
		# Dont link to yourself or anything you want to ignore
		if "\u0394" not in needle and replacement_list[needle] != "":
			broken = False
			for ignore in ignore_list:
				if ignore.lower() in replacement_list[needle].lower() or needle in ignore:
					broken = True
					break
			if broken:
				continue
		# If the needle is in the string, but not in a tag already
		string = try_replace(string, needle, multi)
	return string
