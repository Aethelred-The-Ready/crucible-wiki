...
def try_replace(string, needle, multi=False, override_list=[]):
	if override_list == []:
		override_list = replacement_list
	tr = string
	...
	if sfn != -1 and not in_tag(string, sfn) and not in_tag(string, sfn + len(needle)) and (len(string) == sfn + len(needle) or string[sfn + len(needle)] in " .,:;\"\'"): 
		tr = string.replace(needle, override_list[needle], 1)
	if multi:
		# if multi keep trying till you run out of string
		while sfn + len(override_list[needle]) < len(tr) and needle in tr[sfn + len(override_list[needle]):]:
			sfn = tr.find(needle, sfn + 1)
			# Do not want to replace already in a tag, or middle of a word
			if sfn != -1 and not in_tag(tr, sfn) and not in_tag(tr, sfn + len(needle)) and (len(tr) == sfn + len(needle) or tr[sfn + len(needle)] in " .,:;\"\'"): 
				tr = tr[:sfn] + tr[sfn:].replace(needle, override_list[needle], 1)

	return tr

def run_replaceemnt_list(string, ignore_list=[], multi=False, override_list=[]):
	if override_list == []:
		override_list = replacement_list
	for needle in override_list:
		# Dont link to yourself or anything you want to ignore
		if "\u0394" not in needle and override_list[needle] != "":
			broken = False
			for ignore in ignore_list:
				if ignore.lower() in override_list[needle].lower() or needle in ignore:
					broken = True
					break
			if broken:
				continue
		# If the needle is in the string, but not in a tag already
		string = try_replace(string, needle, multi, override_list)
	return string

# Some files have static compoennts that are added to the autogenerated portion
# They are distinguished by having a _ at the beginning of their file path relative to the path of the autogenerated file
def get_static_content(path):
	pls = path.rfind("/")
	static_path = path[:pls+1] + "_" + path[pls+1:]
	if os.path.isfile(static_path):
		print("Adding static content from " + static_path + " to " + path)
...
replacement_list = {
	...
	"Physician Skill": "[Physician Skill](/docs/skills/background/physician)",
	"Physician Background Skill": "[Physician Background Skill](/docs/skills/background/physician)",
	# Curses
	...
	"Claws": "[Claws](/docs/glossary/claw)",
	"Natural Weapons": "[Claws](/docs/glossary/claw)",
	...
	"Infliction": "[Infliction](/docs/glossary/infliction)",
	...
	"Resist": "[Resist](/docs/combat/defences#resist)",
	...
	"Shield": "[Shield](/docs/combat/defences#shield)",
	...
	"Ward": "[Ward](/docs/combat/defences#ward)",
	# Counted things
	...
	#Skills
	"Diagnose": "[Diagnose](/docs/skills/background/diagnose)",
	"Echo Spellshape": "[Echo Spellshape](/docs/skills/magic/echo_spell)",
	"Empower Spellshape": "[Empower Spellshape](/docs/skills/magic/empower_spell)",
