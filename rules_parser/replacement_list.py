...
def try_replace(string, needle):
    sefn = string.find(needle)
    # Do not want to replace already in a tag, or middle of a word
    if sefn != -1 and not in_tag(string, sefn) and not in_tag(string, sefn + len(needle)) and (len(string) == sefn + len(needle) or string[sefn + len(needle)] == " "): 
